# Hi, I'm Moritz 👋

CTO & Co-Founder at [90/10](https://9010.berlin), based in Berlin.

I build products with TypeScript, mostly around auth, developer tooling, and documentation.

## What I'm working on

- **[better-auth](https://github.com/MauruschatM/better-auth)** – Convex + Better Auth
- **[better-auth-attio-app](https://github.com/MauruschatM/better-auth-attio-app)** – sync Better Auth users & workspaces with Attio
- **[PrisonerJS](https://github.com/MauruschatM/PrisonerJS)** – TypeScript experiments

## Find me

- 🌐 [9010.berlin](https://9010.berlin)
- 🏢 [@neunzig-zehn](https://github.com/neunzig-zehn)

![GitHub stats](https://github-readme-stats.vercel.app/api?username=MauruschatM&show_icons=true&hide_border=true)
